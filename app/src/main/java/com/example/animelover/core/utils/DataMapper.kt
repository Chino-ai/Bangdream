...
object DataMapper {
    fun mapResponsesToEntities(input: List<MemberItem>): List<MemberEntity> {
    ...
    fun mapEntitiesToDomain(input: List<MemberEntity>): List<Member> =
        input.map {
            Member(
                id = it.id,
                birthday = it.birthday,
    ...
    fun mapDomainToEntity(input: Member) = MemberEntity(
        id = input.id,
